...
public class SolutionStepsDialog extends javax.swing.JDialog {
    ...
    private List<SolvingSudoku> states;
    private boolean updateInProgress = false;
    
    public SolutionStepsDialog(java.awt.Frame parent, boolean modal, SudokuDrawPanel drawPanel) {
        super(parent, modal);
    ...
    public void setSteps(SolvingSudoku start, List<SolveStep> steps) {
        this.steps = steps;
        states = generateStates(start, steps);
        stepSlider.setMaximum(steps.size());
        currentStep = 0;
        update();
    }
    ...
    private void update() {
        ...
        drawPanel.setSolution(states.get(currentStep));
        
        if (currentStep < steps.size()) {
            SolveStep step = steps.get(currentStep);
            drawPanel.setPrimaryHighlight(step.primaryCells());
            drawPanel.setSecondaryHighlight(step.secondaryCells());
            descTextArea.setText(step.description());
        } else {
            drawPanel.setPrimaryHighlight(Collections.emptyList());
            drawPanel.setSecondaryHighlight(Collections.emptyList());
    ...
    private void nextButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_nextButtonActionPerformed
        currentStep = Math.min(steps.size(), currentStep + 1);
        update();
    ...
    private List<SolvingSudoku> generateStates(SolvingSudoku start, List<SolveStep> steps) {
        List<SolvingSudoku> result = new ArrayList<>(steps.size() + 1);
        SolvingSudoku sudoku = new SolvingSudoku(start);
        
        result.add(new SolvingSudoku(sudoku));
